...
\begin{center}{\LARGE Distributional Modeling and Lagged Dynamics for EEG-Based Behavioral and Trait Prediction}
\end{center}
...
\ \\[-2mm]
{\bf Keywords:} robust EEG decoding, event-time distribution modeling, behavioral decoding, temporal segmentation, trait prediction
...
Robust EEG-based behavioral decoding remains challenging because of low signal-to-noise ratio and pronounced distribution shift across subjects, sessions, and data releases. We study trial-wise reaction-time prediction from stimulus-locked EEG recordings in the Healthy Brain Network contrast change detection paradigm. Instead of treating reaction time as an ordinary scalar regression target, we reformulate the problem as event-time distribution modeling: models predict a probability distribution over possible response times within the post-stimulus window, are supervised with Gaussian-smoothed temporal targets and distribution-matching losses, and are converted to scalar predictions using a temperature-scaled soft-argmax readout.

This formulation makes the temporal structure of the behavioral target explicit. It aligns supervision with latency variability, enables label-preserving temporal jitter through consistent shifting of the event-time target, and provides distributional uncertainty features that can be used for calibration and stacking. We evaluate the approach under a release-separated protocol using HBN releases R1-R8 for model fitting, R9-R10 for validation, calibration, and meta-learning, and R11 as an untouched labeled test release. We also report the hidden-release CodaBench evaluation from the Foundation of the EEG Foundation Challenge benchmark \citep{EEGFoundationChallengeArxiv2025} as a secondary benchmark comparison.

Across lightweight convolutional and encoder-decoder models, event-time supervision improves over direct scalar regression baselines, and distribution-aware stacking further improves robustness by combining calibrated predictions with trial-wise uncertainty summaries. Under the revised release-separated evaluation, the proposed approach achieves [INSERT MAIN R11 RESULT WITH UNCERTAINTY], compared with [INSERT BEST SCALAR BASELINE RESULT]. These results support the view that, for low-SNR EEG behavioral decoding under cross-subject and cross-release shift, matching the learning objective to the temporal structure of the target can be more important than increasing model capacity alone.
...
\section{Trait Prediction: Externalizing Factor from Lagged EEG Dynamics}
\label{sec:ch2_externalizing}

\subsection{Evaluation Protocol and Window Sampling}

Challenge~2 targets subject-level prediction of the externalizing factor, a continuous psychopathology dimension related
to behavioral patterns. Input samples are 2~s EEG windows sampled at 100~Hz (128 channels), drawn from the CCD task recording of each subject. Each window shares the same subject-level target externalizing score $y_{\mathrm{ext}}$,
and the goal is to predict $\hat y_{\mathrm{ext}}$ from short segments that may occur at arbitrary times within the
session. This setting provides our second instantiation of explicit temporal structure: rather than event timing, we
model stable traits through lagged temporal coupling patterns. To reduce sensitivity to subject- and site-specific
amplitude scaling, per-channel standardization is applied within each window prior to feature extraction
(Figure~\ref{fig:ch2_pipeline}).

Although $y_{\mathrm{ext}}$ is defined at the subject level, we construct training examples by randomly sampling multiple
2~s windows from each subject's CCD recording and treating each window as a noisy observation of the same latent trait.
To avoid leakage, train/validation splits are performed at the subject level (no subject appears in both splits).

\subsection{Lagged Correlation and Transition Representations}
\label{sec:ch2_features}

Externalizing is hypothesized to reflect differences in dynamical regulation rather than instantaneous amplitude
patterns. Accordingly, we seek information in how activity at time $t$ relates to activity at $t+\ell$ across channels.
Lagged correlations summarize time-scale dependent co-fluctuations, while ridge transition matrices provide an oriented
linear mapping that summarizes asymmetric lagged dependence under a linear approximation. This design is motivated by
autoregressive viewpoints in which transition operators encode directed temporal dependencies (e.g., VAR transition
matrices) \citep{geiger2015_causalinf}. Although no causal claims are made here, these lagged descriptors act as
mechanistic proxies for persistence, propagation, and feedback imbalance across the sensor network.

Let $X\in\mathbb{R}^{C\times T}$ denote a per-window normalized EEG segment with $T=200$ time samples.
Let $\Delta t=1/100$~s and consider lags $\ell\in\mathcal{L}$ measured in samples (e.g., $\ell=\{5,10,25,50,100\}$
corresponding to 0.05 to 1.0~s). Two complementary matrix representations are computed for each lag.

\paragraph{Lagged cross-correlation matrices.}
For each lag $\ell$, we compute a time-shifted cross-correlation matrix $C^{(\ell)}\in\mathbb{R}^{C\times C}$ with entries
\begin{equation}
\label{eq:ext_lagcorr}
C^{(\ell)}_{ij}=\mathrm{corr}\!\left(X_{i,1:T-\ell},\,X_{j,1+\ell:T}\right),
\end{equation}
that is, Pearson correlation across time within the window between channel $i$ at time $t$ and channel $j$ at time
$t+\ell$. The diagonal captures within-channel temporal persistence, while off-diagonal entries summarize lag-specific
cross-channel coupling.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{img/ch2_pipeline.png}
\caption{Externalizing prediction pipeline. From a per-window normalized 2~s, 128-channel window, lagged correlations
$C^{(\ell)}=\mathrm{corr}(X_t,X_{t+\ell})$ and ridge transition matrices $A^{(\ell)}$ from
$X_{t+\ell}\approx A^{(\ell)}X_t$ are computed over multiple lags (0.05 to 1~s). Compact scalar summaries are extracted
and a bootstrap add/drop procedure selects stable subsets, after which a ridge regressor outputs $\hat y_{\mathrm{ext}}$.}
\label{fig:ch2_pipeline}
\end{figure}

\paragraph{Ridge transition matrices.}
To summarize lagged dependencies in an oriented linear form, we fit a ridge-stabilized linear transition operator for each
lag $\ell$:
\begin{equation}
\label{eq:ext_ridge_model}
X_{:,1+\ell:T} \approx A^{(\ell)} X_{:,1:T-\ell}.
\end{equation}
We write the estimator in covariance form. Let $X_t=X_{:,1:T-\ell}$ and $X_{t+\ell}=X_{:,1+\ell:T}$, and define
\begin{equation}
\label{eq:ext_cov}
C_{xx}=\frac{1}{T-\ell}X_t X_t^\top,\qquad
C_{xy}=\frac{1}{T-\ell}X_{t+\ell}X_t^\top.
\end{equation}
We use a scale-normalized ridge regularizer with
\begin{equation}
\label{eq:ext_alpha}
\alpha=\mathrm{tr}(C_{xx})/C,
\end{equation}
and obtain the closed-form solution
\begin{equation}
\label{eq:ext_ridge_solution}
A^{(\ell)} = C_{xy}\left(C_{xx}+\lambda\,\alpha\, I\right)^{-1}.
\end{equation}

This normalization makes $\lambda$ less sensitive to overall signal power and improves numerical stability when estimating
$A^{(\ell)}$ from short windows. The diagonal of $A^{(\ell)}$ reflects self-persistence; off-diagonal entries summarize
cross-channel influences; and the antisymmetric component $A^{(\ell)}-(A^{(\ell)})^\top$ provides a compact proxy for
directionality and feedback imbalance (without causal interpretation).

\subsection{Scalar Feature Construction Across Lags}
\label{sec:ch2_scalar_features}

From each lagged correlation matrix $C^{(\ell)}$ and ridge transition matrix $A^{(\ell)}$, we extract a compact set of
scalar summaries to obtain an interpretable, low-dimensional feature vector per lag. This aggregation is motivated by
the weak supervision in Challenge~2: many short windows share a single subject-level label, so we emphasize stable
lagged coupling descriptors that are less sensitive to window-specific noise.

\paragraph{Summaries from lagged correlations.}
For the lagged correlation matrix $C^{(\ell)}$, the mean diagonal value captures within-channel temporal stability.
Global synchronization is summarized by the mean absolute correlation across all entries, and inter-channel coupling is
summarized by the mean off-diagonal correlation (reported both as signed mean and as mean absolute value when used as a
magnitude descriptor).

To quantify whether coupling is concentrated or diffuse, we compute the entropy of normalized absolute entries. For a
matrix $M\in\mathbb{R}^{C\times C}$, let $v=\mathrm{vec}(|M|)$ and $p=v/\sum_k v_k$ (if $\sum_k v_k>0$). We define
\begin{equation}
\label{eq:ext_entropy}
H(M)=-\sum_{k:\,p_k>0} p_k \log p_k,
\end{equation}
so lower entropy indicates a more peaked interaction pattern.

\paragraph{Summaries from ridge transitions.}
For the lagged transition matrix $A^{(\ell)}$, self-persistence is captured by the mean diagonal value, and overall
influence strength by the mean absolute entry magnitude. Cross-channel effects are summarized by the mean off-diagonal
value, and the overall magnitude of the lagged mapping by the Frobenius norm $\|A^{(\ell)}\|_F$ as a proxy for mapping
strength. To reflect directionality and feedback imbalance, we use the magnitude of the antisymmetric component
$A^{(\ell)}-(A^{(\ell)})^\top$ (without causal interpretation).

Finally, we include a sparsity ratio defined as the fraction of entries below a small magnitude threshold:
\begin{equation}
\label{eq:ext_sparsity}
\mathrm{sparsity}_{\theta}(A^{(\ell)})=\frac{1}{C^2}\sum_{i=1}^{C}\sum_{j=1}^{C}\mathbf{1}\left(|A^{(\ell)}_{ij}|<\theta\right),
\end{equation}
with $\theta=0.05$ in our implementation.

Across lags, these descriptors summarize how activity persists and propagates across channels as a function of time
scale, while remaining comparatively robust to nuisance amplitude variation.

\paragraph{Implementation details.}
All features are computed on per-window standardized signals (channel-wise z-scoring within each 2~s segment). We use the
lag set $\mathcal{L}=\{5,10,25,50,100\}$ (0.05 to 1.0\,s at 100\,Hz) and a ridge coefficient $\lambda$ fixed across lags.
To reduce redundancy between highly overlapping windows and control computational cost, windows are subsampled with a
fixed stride during feature extraction.

\subsection{Bootstrap-Driven Feature Selection and Ridge Regression}
\label{sec:ch2_selection}

Given lagged temporal-coupling descriptors across multiple lags, we train a ridge regression model using a
stability-oriented feature selection procedure. Subjects are first partitioned such that the mean and standard deviation
of the externalizing target are approximately matched across folds, ensuring comparable target distributions during
selection and evaluation. An initial feature pool is created by ranking candidates via absolute Pearson correlation with
$y_{\mathrm{ext}}$ on the training subjects.

Feature selection is performed with a stochastic add/drop search coupled to bootstrap validation. At each iteration,
training subjects are resampled with replacement (bootstrap over subjects rather than windows), a ridge regressor is fit
on the current feature subset, and predictions are generated for a fixed subject-level validation split that is not
resampled during bootstrapping. Window-level predictions are aggregated to subject-level scores before computing nRMSE,
matching the evaluation target.

This yields a distribution of nRMSE values across bootstraps. Candidate feature-set updates are accepted only if they
(i) improve median nRMSE and (ii) reduce variability across bootstraps (measured by the standard deviation of nRMSE),
favoring subsets that are both accurate and stable. The procedure is repeated until a stopping criterion is met (no
accepted update for a fixed number of iterations), after which the final ridge model is refit on the selected feature
subset and used for inference.

By representing each 2~s EEG window through lag-dependent correlation structure and regularized linear transitions, the
externalizing prediction task is reduced to learning from compact descriptors of temporal stability, coupling strength,
asymmetry, and sparsity across multiple physiologically relevant timescales (50~ms to 1~s). Because the descriptors are
computed on standardized signals and rely on regularized estimators, sensitivity to site-specific amplitude scaling is
reduced, supporting generalization to unseen subjects and heterogeneous dataset releases.

\subsection{Externalizing Factor Prediction Results}
\label{sec:ext_results}



We first compared against end-to-end EEG baselines trained directly on randomly sampled 2~s CCD segments. We evaluated
several off-the-shelf architectures from Braindecode \citep{braindecode}, including EEGNet \citep{lawhern2018eegnet} and
EEGConformer \citep{song2023eegconformer}, trained from scratch with mean-squared error and early stopping based on
validation nRMSE. Across architectures and reasonable hyperparameter settings, these end-to-end baselines tended to
collapse toward near-constant predictions (close to the training-set mean), yielding performance near the mean-predictor
reference (nRMSE $\approx 1$). This behavior is consistent with the weak supervision in Challenge~2, where many short
windows share the same subject-level label and window-level SNR is low. In contrast, our lagged dynamics descriptor
pipeline provides an explicit temporal-structure representation that yields a more stable signal for trait prediction.

\begin{table}[t!]
\caption{Externalizing factor prediction results on the official held-out evaluation (lower nRMSE is better; nRMSE $=1$
corresponds to predicting the mean target).}
...
\hline
\textbf{Method} & \textbf{Externalizing nRMSE} $\downarrow$ \\
\hline
End-to-end Braindecode baselines (trained from scratch) & $\approx 1.00$ \\
Regression SneddyNet (direct window regression) & 1.0080 \\
\textbf{NeuroSned (lagged EEG dynamics)} & \textbf{0.98519} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:results_ext} compares our final lagged EEG dynamics pipeline against (i) a lightweight convolutional
regression baseline (SneddyNet) trained directly on 2~s windows and (ii) representative end-to-end Braindecode baselines
trained from scratch.




Overall, explicitly modeling lagged temporal coupling yields the most reliable improvements in this low-SNR,
subject-level setting, where end-to-end regression on short windows provides limited effective supervision.

...
The core event-time experiments are designed to isolate the role of output formulation. The regression controls above test how far scalar supervision can go with task-specific temporal readouts and external EEG backbones. Here, we fix the event-time segmentation U-Net (ETS-U-Net) backbone and vary the event-time objective, so that differences in scalar accuracy, posterior geometry, and shifted-crop behavior reflect how RT is supervised over time rather than which architecture is used.
...
For each trial window, the ground-truth event time is represented as a relative time
...
Table~\ref{tab:r11_segmentation_losses} compares the paper-facing ETS-U-Net objective variants over five seeds under the fixed backbone and release-separated protocol. The CE and likelihood-family objectives form the strongest scalar-readout group after readout-temperature tuning, with R11 nRMSE between 0.8745 and 0.8778. This improves over the strongest scalar regression baseline, ETR-CNN large (0.8928 $\pm$ 0.0042; Table~\ref{tab:r11_regression_baselines}), while the time-only control remains much closer to the scalar-regression regime. The Wasserstein control is weaker as a scalar predictor before tuning, but remains useful diagnostically because it produces the largest localizer-like fraction in shifted-crop evaluation.

\begin{table}[!t]
\centering
\caption{Five-seed comparison of event-time objectives with the fixed ETS-U-Net backbone.}
...
\toprule
\textbf{Objective} & \textbf{Supervision} & \textbf{Valid nRMSE} $\downarrow$ & \textbf{R11 nRMSE} $\downarrow$ & \textbf{Tau-tuned nRMSE} $\downarrow$ & \textbf{Shift slope} & \textbf{Localizer-like} \\
\midrule
CE & soft target & 0.8763 $\pm$ 0.0044 & 0.8774 $\pm$ 0.0044 & 0.8753 $\pm$ 0.0039 & $-0.339$ $\pm$ 0.017 & 0.209 $\pm$ 0.031 \\
EventNLL & latent-event likelihood & 0.8769 $\pm$ 0.0030 & 0.8805 $\pm$ 0.0021 & 0.8772 $\pm$ 0.0018 & $-0.344$ $\pm$ 0.014 & 0.221 $\pm$ 0.026 \\
Mixture EventNLL & two-scale likelihood & 0.8744 $\pm$ 0.0018 & 0.8785 $\pm$ 0.0047 & 0.8745 $\pm$ 0.0053 & $-0.355$ $\pm$ 0.024 & 0.246 $\pm$ 0.042 \\
Hazard EventNLL & hazard likelihood & 0.8755 $\pm$ 0.0027 & 0.8776 $\pm$ 0.0031 & 0.8778 $\pm$ 0.0041 & $-0.328$ $\pm$ 0.020 & 0.196 $\pm$ 0.042 \\
Time-only & scalar readout control & 0.8944 $\pm$ 0.0048 & 0.8943 $\pm$ 0.0025 & 0.8917 $\pm$ 0.0046 & $-0.301$ $\pm$ 0.043 & 0.160 $\pm$ 0.066 \\
Wasserstein & CDF-distance control & 0.8997 $\pm$ 0.0035 & 0.8995 $\pm$ 0.0078 & 0.8896 $\pm$ 0.0033 & $-0.337$ $\pm$ 0.044 & 0.271 $\pm$ 0.065 \\
\bottomrule
\end{tabular}%
}
\end{table}

...
Our event-time experiments use a fixed ETS-U-Net backbone (event-time segmentation U-Net), which maps an EEG trial window
$X\in\mathbb{R}^{C\times T}$ to per-time logits $z_{0:T-1}$ (Figure~\ref{fig:ets_unet}). We keep the backbone fixed across
loss ablations so that differences in scalar accuracy, posterior geometry, and shifted-crop behavior can be attributed to
the output objective rather than to architecture changes. ETS-U-Net follows a 1-D U-Net style encoder-decoder with skip

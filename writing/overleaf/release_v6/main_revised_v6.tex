...
To avoid subject leakage, all models use the same release-separated CCD split: R1--R8 for fitting, R9--R10 for development choices such as early stopping, checkpoint selection, temperature tuning, optional calibration, and protocol diagnostics where applicable, and R11 for one-shot final reporting after all model and readout choices are fixed. R11 labels are not used for model fitting, checkpoint selection, temperature selection, calibration, stacking, or protocol decisions. For consistency, the main analyzed set includes only trials whose response lies inside the fixed 2~s window (\(0.5 \leq \mathrm{RT} \leq 2.5\)~s). Table~\ref{tab:ccd_protocol_stats} reports the resulting partitions; metadata checks confirmed zero subject overlap across train, development, and R11 test after filtering.
...
\subsection{Shared Training, Calibration, and Inference Protocol}
...
Within this fixed protocol, the main experimental axes are deliberately narrow (Table~\ref{tab:shared_protocol_by_family}). The regression block varies the backbone and scalar/temporal readout family. The event-time block uses the same fixed 0.5--2.5~s crop but varies how RT is represented in the objective: as a soft crop-relative event-time target or as an observation under a latent event-time likelihood. Event-time analyses use the 5~s stimulus-locked cache only for fixed-crop extraction and shifted-crop diagnostics; the main training runs do not use random crop jitter. When temperature scaling is used for event-time posterior readout, \(\tau\) is selected on R9--R10 to minimize development nRMSE and then applied unchanged to R11.

\begin{table}[!t]
\centering
\caption{Model-family-specific targets and readouts within the shared fixed-crop protocol.}
...
\toprule
\textbf{Model family} & \textbf{Target or supervision} & \textbf{Readout / calibration} \\
\midrule
...
The regression baselines establish how far scalar supervision can go before introducing event-time targets. We use three complementary controls: a compact segment-pooling regressor (MSP-CNN), a matched temporal-readout regressor (ETR-CNN), and supervised external EEG backbones trained from scratch. Together, these baselines test whether performance is explained by architecture capacity or differentiable expectation readout before attributing gains to event-time distribution supervision.
...
(i) a learned channel-squeeze layer ($1\times1$ conv, 128$\rightarrow$48) that forms data-driven electrode mixtures,
and (ii) an EfficientNet-like temporal hierarchy with widening stages of residual depthwise-separable blocks
(48$\rightarrow$96$\rightarrow$192 channels) and anti-aliased temporal downsampling.
...
ETR-CNN is the matched temporal-readout counterpart. It keeps scalar RT supervision, but avoids collapsing the temporal feature map with segment statistics before prediction. Instead, a dilated depthwise-separable backbone preserves a time-indexed representation, predicts weights over crop-relative time bins, and outputs RT as the expected time under those weights. Thus ETR-CNN tests whether a differentiable expectation readout and expanded temporal receptive field are sufficient without event-time posterior supervision. ETR-CNN large keeps the same readout while increasing feature capacity.

To contextualize these task-specific regressors against established EEG backbones, we also evaluate external architectures adapted to scalar RT regression. These include classical convolutional EEG baselines, Deep4Net and ShallowFBCSPNet \citep{braindecode}, the compact EEGNet architecture \citep{lawhern2018eegnet}, TIDNet's thinker-invariant convolutional design \citep{kostas2020thinkerinvariance}, the convolutional-transformer EEGConformer \citep{song2023eegconformer}, the attention temporal convolutional ATCNet architecture \citep{altaheri2022atcnet}, EEGPT \citep{wang2024eegpt}, Medformer \citep{wang2024medformer}, and the LaBraM foundation-style architecture \citep{Jiang2024LaBraM}. All external architectures are trained from scratch; pretrained weights are not used. The main tables report the wrapped/standardized variants, with unwrapped external runs treated as sanity checks in the Appendix.
...
\paragraph{Regression baseline results.}
Table~\ref{tab:r11_regression_baselines} reports direct-regression and external-backbone baselines repeated over five seeds. The ETR-CNN large capacity variant is the strongest direct-regression baseline, while the compact MSP-CNN and base ETR-CNN models remain competitive. Wrapped external architectures are substantially better controlled than their unwrapped sanity-check counterparts, but remain behind the task-specific regression baselines on R11.

\begin{table}[!t]
\centering
\caption{Direct-regression and wrapped external backbone baselines. Values are mean $\pm$ sample standard deviation over seeds 2025--2029.}
...
Despite strong performance and fast training, direct regression does not explicitly supervise where within the analysis
window the response-related event occurs. This limitation motivated the
...
Temporal-readout regression separates two ideas that are otherwise coupled in event-time segmentation: using an expectation-style readout and supervising a full event-time distribution. In ETR-CNN, the learned time-bin weights are only a readout parameterization for scalar RT regression. The model is optimized through scalar RT error, not by matching a crop-relative event-time posterior. This makes ETR-CNN a control for whether the gains below can be explained by soft-argmax-style coordinate regression alone \citep{nibali2018dsnt}, with dilated convolutions providing temporal context without changing the scalar-supervised objective \citep{yu2016dilated}.

The results in Table~\ref{tab:r11_regression_baselines} show that temporal-readout regression improves over scalar segment pooling: ETR-CNN reaches a test nRMSE of 0.8977 $\pm$ 0.0068 versus 0.8998 $\pm$ 0.0080 for MSP-CNN, and the larger ETR-CNN variant improves further to 0.8928 $\pm$ 0.0042. This indicates that temporal capacity and expectation-style readout are useful direct-regression controls. However, the model is still trained through scalar RT error rather than to match the geometry of an event-time posterior. These considerations motivate the segmentation formulation below, which makes response timing explicit via event-time supervision and distribution-matching losses.

\subsection{Event-Time Segmentation Framework}
\label{sec:rt_segmentation}

To make response timing explicit, we reformulate RT prediction as event-time posterior modeling. Instead of predicting a scalar directly, the model produces per-time logits and a posterior distribution over response-relevant event times; scalar RT is then read out as the posterior expectation. The key distinction from temporal-readout regression is that the objective directly supervises the distributional event-time representation, either through a soft target or through a latent-event likelihood, rather than using a temporal head only as a scalar-regression readout.
...
The ETS-U-Net family is used to map an EEG trial window

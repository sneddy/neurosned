...
\subsection{Event-Time Segmentation Architectures}
...
Our primary event-time segmentation architecture is ETS-U-Net, which maps an EEG trial window
...
Table~\ref{tab:rt_seg_models} summarizes the architecture family. ETS-U-Net provides the main template, while attention,
EEG-Inception, and factorized variants introduce complementary temporal and cross-channel inductive biases for ablation and
secondary ensembling.
...
These variants share the per-time-logit interface and differ only in architectural bias; the output objectives are defined
in the next subsection. The post-hoc distribution-aware stacking procedure used for the secondary ensemble analysis is
described in Appendix~\ref{sec:rt_stacking}.

\subsection{Distributional Supervision Objectives}

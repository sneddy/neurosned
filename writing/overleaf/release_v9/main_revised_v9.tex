...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.88\linewidth]{img/ch1/trial_sorted_posterior_raster_calibrated.png}
...
We first inspect the trial-sorted posterior raster in Figure~\ref{fig:posterior_raster_main} before reducing the posterior to summary metrics. This dataset-level view shows how each loss distributes probability mass across the represented RT range, making posterior width, target alignment, and loss-specific confidence patterns visible. Figure~\ref{fig:posterior_geometry_main} and Table~\ref{tab:posterior_geometry_metrics} then summarize these differences quantitatively.

Table~\ref{tab:posterior_geometry_metrics} uses four groups of metrics. First, scalar nRMSE and MAE measure posterior-mean RT prediction. Second, CRPS and fixed-kernel EventNLL score the full posterior distribution; the fixed-kernel EventNLL uses the same Gaussian observation kernel for all models, so it evaluates each predicted posterior under a common likelihood score rather than under its own training loss. Third, Width80, Mass $\pm$150 ms, and mode-mean gap describe localization and posterior concentration: Width80 is the median width of the nominal central 80\% posterior interval, Mass $\pm$150 ms is the posterior probability assigned within 150 ms of the observed RT, and mode-mean gap measures disagreement between the posterior peak and the posterior mean used for scalar readout. Finally, Coverage80 and Coverage MAE summarize interval behavior. Coverage80 is the fraction of trials whose observed RT falls inside the nominal central 80\% posterior interval. Coverage MAE is the mean absolute deviation between nominal and empirical coverage across central intervals $\{50\%,60\%,70\%,80\%,90\%\}$. Together, these metrics separate distributional accuracy, temporal concentration, target alignment, and interval reliability.
...
These diagnostics show that losses with similar scalar error can induce different posterior semantics. CE+time and CE give strong scalar readouts but broad, conservative posteriors: their nominal 80\% intervals cover about 90\% of trials. EventNLL produces a sharper and more target-concentrated posterior, with the narrowest Width80, the highest near-target mass, and the best fixed-kernel EventNLL score, but it under-covers nominal intervals. Wasserstein gives the best coverage trade-off, with the smallest Coverage MAE and Coverage80 closest to 0.80, although with weaker scalar nRMSE and less target-aligned mass. Thus, posterior geometry reveals trade-offs among scalar accuracy, target concentration, and interval coverage that scalar nRMSE alone would hide.
...
\begin{table}[!hbp]
\centering
\caption{Shifted-crop diagnostic and shift-jitter intervention on the 5~s holdout dataset. Values are mean $\pm$ standard deviation across seeds. Holdout \(\tau\)-nRMSE uses the canonical fixed window; shifted relative nRMSE is computed on shifted-crop examples in which RT remains observable within the evaluated 2~s window; sensitivity and direction are computed on the common-inside subset.}

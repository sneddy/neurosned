...
We evaluate the method under a release-separated protocol using HBN releases R1--R8 for model fitting, R9--R10 for development-stage calibration and meta-learning, and R11 for final reporting. The original hidden-release CodaBench evaluation is reported separately for continuity with the challenge setting. Under this protocol, the calibrated CE+time event-time segmentation model achieves 0.935573 nRMSE on R11 [0.918136, 0.954309], compared with 0.945938 [0.930153, 0.963795] for the strongest direct-regression baseline.
...
Table~\ref{tab:r11_segmentation_losses} summarizes the event-time segmentation loss ablations. The calibrated hybrid event-time segmentation model reaches 0.935573 nRMSE on R11, improving over the strongest direct-regression baseline. CE-only soft-label supervision is also strong. This is the central empirical step in the narrative: once RT is represented as an event-time posterior, distributional supervision over time becomes more useful than treating RT as a scalar window label.
...
\begin{table}[!t]
\centering
\caption{Quantitative posterior geometry on R11 for matched event-time segmentation losses. Scalar nRMSE and MAE summarize point-readout accuracy; CRPS and fixed-kernel EventNLL are distributional scores computed from the full posterior; width, near-target mass, mode-mean gap, and empirical coverage quantify posterior concentration and calibration. CRPS is reported in milliseconds. Fixed-kernel EventNLL uses the same Gaussian observation kernel for all models ($\sigma=0.15$~s), so lower values indicate higher likelihood of the observed RT under the predicted event-time mixture.}
...
\subsection{Distribution-Aware Stacking as a Secondary Ensemble Analysis}

The R11 results above are intentionally reported without post-hoc stacking, so that the main comparisons isolate the event-time objective, scalar readout, and posterior geometry. The available stacking evidence comes from the original hidden-release CodaBench evaluation (Appendix Table~\ref{tab:appendix_codabench}). In that submission-oriented pipeline, gradient boosting stacking achieved 0.91394 hidden nRMSE, compared with 0.92334 for the best single model, 0.91840 for manual blending, and 0.91539 for Ridge stacking. This supports the role of posterior-derived meta-features as an ensemble calibration layer, while the release-separated R11 claims remain based on the single-model comparisons in Tables~\ref{tab:r11_regression_baselines}--\ref{tab:posterior_geometry_metrics}.
...
Unwrapped external architectures are useful for diagnosing scale sensitivity. Because our own models use per-window standardization, the primary comparison uses wrapped external models.

...
RT is a noisy realization of a latent event time. Compact extensions with a two-scale Gaussian observation kernel and a
hazard/survival parameterization show that the event-time formulation is not tied to one softmax-Gaussian implementation.

This framing is useful for brain-behavior decoding when targets are inherently temporal. It aligns supervision with latency variability, enables latent-consistent jittered cropping, and makes posterior geometry inspectable. In our experiments, scalar scores alone hide important differences in posterior width, mode-mean disagreement, target-aligned mass, and coverage. A shifted-crop diagnostic further shows that event-time segmentation moves predictions in a more event-like direction than scalar regression, while also revealing that fixed-window training still permits stimulus-locked shortcut behavior. We therefore treat temperature calibration and posterior regularization primarily as geometry analysis and control, not as the central method claim.


We evaluate the method under a release-separated protocol using HBN releases R1--R8 for model fitting, R9--R10 for development-stage calibration and meta-learning, and R11 for final reporting. The original hidden-release CodaBench evaluation is reported separately for continuity with the challenge setting. Under this protocol, the calibrated CE+time event-time segmentation model achieves 0.935573 nRMSE on R11 [0.918136, 0.954309], compared with 0.945938 [0.930153, 0.963795] for the strongest direct-regression baseline. Five-seed robustness checks on selected core recipes preserve the same ordering: CE and EventNLL segmentation remain about 0.010--0.011 nRMSE better than the repeated temporal-readout regression baseline, while the time-only control is much closer to scalar regression.
...
\begin{enumerate}
    ...
    \item We verify the main event-time supervision gap with five-seed robustness checks over selected scalar, external, and segmentation baselines.
    \item We formulate EventNLL as a principled latent event-time likelihood for RT, complementing soft Gaussian-label supervision, and evaluate Gaussian-mixture and hazard/survival extensions.
    \item We analyze posterior geometry and a shifted-crop diagnostic to separate scalar accuracy from event-time localization behavior.
    \item We treat temperature calibration and secondary distribution-aware stacking as readout/ensemble analyses while reporting the original hidden challenge results only as secondary evidence.
\end{enumerate}
...
\subsection{Seed Robustness of the Main Gap}
...
\subsection{Shifted-Crop Diagnostic: Partial Event Localization}

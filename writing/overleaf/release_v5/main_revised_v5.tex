...
\ \\[-2mm]
{\bf Keywords:} weak event-time supervision, event-time posterior modeling, EEG reaction-time decoding, behavioral latency, posterior geometry
...
\begin{enumerate}
    ...
    \item We use posterior geometry to separate scalar accuracy from temporal evidence, uncertainty, target concentration, and calibration.
\end{enumerate}
...
We use the reaction-time prediction task introduced by the NeurIPS EEG Foundation Challenge \citep{EEGChallengeWebsite2025,EEGFoundationChallengeArxiv2025}, based on curated releases of the HBN-EEG dataset \citep{Shirazi2024HBNEEG}. In this contrast change detection (CCD) task, participants viewed two flickering grating stimuli and responded after a contrast change made one grating perceptually dominant. The released target is the trial-wise reaction time from stimulus onset to button press.

Each model input is a stimulus-locked EEG segment represented as 128 EEG channels sampled at 100~Hz after excluding the reference channel. The fixed analysis window spans 0.5--2.5~s after stimulus onset (Figure~\ref{fig:tasks}). For each trial, models predict RT in seconds, and performance is evaluated using normalized RMSE (nRMSE), defined as RMSE divided by the standard deviation of the true targets within the evaluated split \citep{EEGChallengeWebsite2025}.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\linewidth]{img/challenge_task.png}
    \caption{HBN-EEG contrast change detection reaction-time task. Each trial provides a stimulus-locked EEG segment; the fixed analysis window used here spans 0.5--2.5~s after stimulus onset with 128 EEG channels sampled at 100~Hz. The released target is the trial-wise reaction time from stimulus onset to button press.}
    \label{fig:tasks}
\end{figure}
...
We use this task as a controlled evaluation setting for separating three possible sources of RT prediction performance: EEG backbone capacity, temporal readout parameterization, and distributional event-time supervision. The benchmark therefore fixes the data split, target support, input representation, and scalar evaluation rule before varying architectures and objectives in the Methods.

To avoid subject leakage, all models use the same release-separated CCD split: R1--R8 for fitting, R9--R10 for development choices such as early stopping, checkpoint selection, and temperature tuning, and R11 for final reporting after all model and readout choices are fixed. For consistency, the analysis includes only 2~s windows containing the response event (\(0.5 \leq \mathrm{RT} \leq 2.5\)~s). Table~\ref{tab:ccd_protocol_stats} reports the resulting partitions; metadata checks confirmed zero subject overlap across train, development, and test after filtering.
...
All neural models receive the same input normalization: for each 2~s trial window and EEG channel, we subtract the channel's temporal mean within the window and divide by its temporal standard deviation within the same window. The training recipe is also shared: Adam with learning rate \(10^{-3}\), zero weight decay, train batch size 128, validation batch size 256, up to 100 epochs, early stopping on development nRMSE with patience 20, and no mixup. Unless otherwise stated, training uses one fixed mild augmentation profile so that regularization is part of the protocol rather than a model-specific tuning degree of freedom. Channel dropout is applied with probability 0.25 and removes at most 30\% of channels, temporal cutout is applied with probability 0.25 and masks a contiguous 10--50 sample interval, and Gaussian noise is applied with probability 0.3 and standard deviation \(0.01 + U(0,0.01)\). These perturbations approximate plausible electrode loss, brief local signal corruption, and measurement noise while preserving the trial-wise RT label.

Within this fixed protocol, the paper-facing experimental axes are deliberately narrow. The regression block varies the backbone and scalar/temporal readout family. The event-time segmentation block fixes the ETS-U-Net backbone for the main loss ablations and varies the objective or posterior parameterization: soft-label CE, CE+time, EventNLL latent likelihood, time-only and Wasserstein controls, a Gaussian-mixture observation kernel, and a survival-style hazard posterior.
...
We begin with baselines that keep the shared protocol in Section~\ref{sec:dataset_protocol} fixed and vary only how the EEG window is mapped to scalar RT. Direct scalar models output RT directly, whereas temporal-readout regression models produce a time-indexed readout but remain supervised by scalar RT error.
...
Because normalization, augmentation, optimizer settings, batch sizes, early stopping, and evaluation are fixed in the dataset protocol, differences among these regression rows are intended to be architecture and readout differences.
...
of the task. Even when a temporal readout is present, supervision is still applied through scalar RT error rather than
through a posterior over response time. This limitation motivated the segmentation reformulation in
Section~\ref{sec:rt_segmentation}, which makes response timing explicit via an event-time distribution.
...
To make event timing explicit while keeping the data protocol fixed, RT prediction was reformulated as
\emph{temporal segmentation}: instead of regressing a scalar directly, we predict a distribution over time bins in the same
2~s crop and read out RT as its expectation (Figure~\ref{fig:ch1_pipeline}). Compared to distributional readouts used only as a final head, the key difference here is
that supervision itself is defined over time coordinates. In the reported main ablations, the crop start is fixed to the
standard 0.5--2.5~s window; the methodological change is the internal target representation and loss, not a different
sampling protocol. This is our RT instantiation of explicit temporal structure: behavior is modeled through event timing
rather than static regression.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{img/ch1/ch1_pipeline.png}
    \caption{Temporal segmentation pipeline for RT prediction. For each trial, the standard 2~s crop is represented in crop-relative coordinates and the observed RT is converted into either a Gaussian soft target $q_t$ or a latent-event observation for EventNLL. Per-time logits $z_t$ are produced by a 1-D segmentation model and optimized with distribution-matching losses (e.g., soft-label CE, CDF-based Wasserstein-1, KL, and mild entropy penalty) or EventNLL objectives. The main readout is a temperature-scaled softmax posterior; survival-style hazard readouts and alternate observation kernels are evaluated as compact extensions. RT is obtained by temperature-scaled soft-argmax, with $\tau^\star$ selected to minimize validation nRMSE.}
    \label{fig:ch1_pipeline}
\end{figure}

\paragraph{Notation and target construction.}
Let a 2~s crop be $X \in \mathbb{R}^{C\times T}$ with $\mathrm{sfreq}=100$~Hz, $T=200$ and $\Delta t=1/\mathrm{sfreq}$.
For the main ablations, training, validation, and official inference all use the fixed stimulus-locked 2~s crop with offset
$t_0=0.5$~s (0.5--2.5~s). The event-time target is therefore just the observed absolute RT shifted into crop coordinates,
$y_{\mathrm{rel}}=y_{\mathrm{abs}}-t_0$, and predictions are converted back by
...
For each crop, the ground-truth event time within the crop is represented as a relative time
...
As a readout extension, we also evaluate a survival-style hazard parameterization. Instead of using logits only as unconstrained softmax scores, each temporal logit defines a conditional event probability
\begin{equation}

...
The shifted-crop diagnostic asks whether posterior predictions respond systematically when the temporal frame of reference changes. Models trained on a fixed stimulus-locked window may rely on information that predicts RT without representing response timing in crop-relative coordinates. Consistent movement opposite to the crop shift therefore provides evidence of crop-relative prediction behavior, whereas limited movement indicates continued dependence on the canonical temporal frame.

Shift-jitter training improves shifted-crop robustness and increases the frequency of movement in the expected direction, while sensitivity remains below full shift equivariance. The diagnostic should therefore be interpreted as a controlled stress test of temporal behavior rather than as direct validation against an independently observed neural-event time. In addition, candidate training crop starts are clipped when necessary to keep the observed RT inside the window. The realized crop-start distribution is consequently RT-dependent near the temporal boundaries, and the intervention combines temporal-frame variation with target-aware crop construction.

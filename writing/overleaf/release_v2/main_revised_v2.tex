...
Robust EEG decoding is often framed as a backbone problem: larger architectures or pretrained representations are expected to absorb low signal-to-noise ratio and cross-subject variability. We study a complementary formulation-first route for trial-wise reaction-time (RT) prediction from Healthy Brain Network EEG in the contrast change detection paradigm. Under a subject-disjoint, release-separated evaluation, a compact task-specific regression model already outperforms heavier wrapped external EEG backbones, suggesting that architecture scale alone is not the main bottleneck in this setting.

We then ask whether adding temporal structure to the output helps. A temporal-readout regressor parameterizes an expectation over time bins while retaining scalar RT supervision, but it yields only marginal gains over compact scalar regression. This negative result motivates our main reformulation: instead of predicting a single RT directly, the model predicts an event-time posterior over possible response latencies and reads out RT by expectation. Soft-label event-time segmentation improves over the strongest regression baselines, and loss ablations show that the gain comes from distributional supervision rather than from the soft-argmax readout alone. We also evaluate EventNLL, a latent event-time likelihood that treats observed RT as a noisy realization of an unobserved response-relevant event time.

This posterior formulation makes model behavior inspectable beyond scalar nRMSE. Posterior width, target-aligned mass, mode-mean disagreement, and interval coverage reveal differences hidden by point predictions, while a shifted-crop diagnostic shows that event-time segmentation moves predictions in a more event-like direction than scalar regression, though fixed-window shortcuts remain. On R11, calibrated CE+time segmentation reaches 0.935573 nRMSE [0.918136, 0.954309], compared with 0.945938 [0.930153, 0.963795] for the strongest direct-regression baseline; five-seed checks preserve the same ordering. Together, these results support event-time distribution modeling as a lightweight route to more robust EEG-based RT prediction.
...
Motivated by the success of foundation models in language and vision, the EEG community has recently seen rapid growth in pretrained backbones that aim to learn transferable, task-agnostic representations from heterogeneous EEG corpora, including transformer, state-space, and cross-setup pretraining approaches \citep{wang2024eegpt,Jiang2024LaBraM,Chen2024EEGFormer,Wang2025EEGMamba,ElOuahidi2025REVE,Yue2024BrainGPT}. Overall, these efforts suggest that large-scale self-supervised pretraining can reduce reliance on task-specific labels and improve performance.

Despite this progress, robust EEG decoding remains difficult because task-relevant signals are low-SNR and strongly affected by artifacts, nonstationarity, and inter-subject variability. These factors often yield models that perform well within a subject, session, or curated split but degrade under cross-subject, cross-site, or release-separated evaluation.
...
\begin{enumerate}
    \item We reformulate EEG-based reaction-time prediction as event-time distribution modeling, replacing ordinary scalar regression with a posterior over response latencies and an expectation-based scalar readout.
    \item We evaluate this formulation against compact scalar baselines, temporal-readout regression, and standardized external EEG architectures under a release-separated HBN protocol with R11 subject-bootstrap confidence intervals.
    \item We show through loss ablations and five-seed robustness checks that distributional event-time supervision improves over scalar soft-argmax regression, and we introduce EventNLL as a latent event-time likelihood alternative to soft Gaussian-label supervision.
    \item We analyze posterior geometry and shifted-crop behavior to separate scalar accuracy from uncertainty, target alignment, and partial event-localization behavior; temperature tuning and distribution-aware stacking are treated as secondary readout/ensemble analyses.
\end{enumerate}

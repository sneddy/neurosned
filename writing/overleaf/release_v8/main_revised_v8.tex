...
To avoid subject leakage, all models use the same release-separated CCD split: R1--R8 for fitting, R9--R10 for development choices such as early stopping, checkpoint selection, readout-temperature tuning, and protocol diagnostics where applicable, and R11 as a one-shot final holdout after all model and readout choices are fixed. R11 holdout labels are not used for model fitting, checkpoint selection, readout-temperature selection, stacking, or protocol decisions. For consistency, the main analyzed set includes only trials whose response lies inside the fixed 2~s window (\(0.5 \leq \mathrm{RT} \leq 2.5\)~s). Table~\ref{tab:ccd_protocol_stats} reports the resulting partitions; metadata checks confirmed zero subject overlap across train, development, and holdout after filtering.
...
\addlinespace[0.15em]
CE & Gaussian soft label & \mbox{0.8763 $\pm$ 0.0044} & \textbf{\mbox{0.8774 $\pm$ 0.0044}} & \mbox{0.8753 $\pm$ 0.0039} \\
\midrule
\multicolumn{5}{@{}l}{\textit{Likelihood-based objectives}} \\
\addlinespace[0.15em]
EventNLL & Gaussian RT \mbox{likelihood} & \mbox{0.8769 $\pm$ 0.0030} & \mbox{0.8805 $\pm$ 0.0021} & \mbox{0.8772 $\pm$ 0.0018} \\
Mixture EventNLL & two-scale RT \mbox{likelihood} & \textbf{\mbox{0.8744 $\pm$ 0.0018}} & \mbox{0.8785 $\pm$ 0.0047} & \textbf{\mbox{0.8745 $\pm$ 0.0053}} \\
Hazard EventNLL & hazard posterior + \mbox{likelihood} & \mbox{0.8755 $\pm$ 0.0027} & \mbox{0.8776 $\pm$ 0.0031} & \mbox{0.8778 $\pm$ 0.0041} \\
\midrule
\multicolumn{5}{@{}l}{\textit{Scalar-readout and distance controls}} \\
\addlinespace[0.15em]
Time-only & posterior-mean RT loss & \mbox{0.8944 $\pm$ 0.0048} & \mbox{0.8943 $\pm$ 0.0025} & \mbox{0.8917 $\pm$ 0.0046} \\
Wasserstein & CDF match to soft label & \mbox{0.8997 $\pm$ 0.0035} & \mbox{0.8995 $\pm$ 0.0078} & \mbox{0.8896 $\pm$ 0.0033} \\
\bottomrule
\end{tabularx}
\end{table}
...
The event-time models return a posterior over response-relevant time bins, whereas the benchmark metric is scalar RT error. We therefore read out RT as the posterior expectation,
\begin{equation}
...
Different objectives can produce posteriors with different sharpness, so the raw posterior mean is not always the fairest scalar readout. We use post-hoc temperature scaling \citep{guo2017calibration} only as a readout-tuning step: for each model family, \(\tau\) is selected on R9--R10 to minimize development nRMSE of the posterior-mean readout,
...
The selected value is then applied unchanged to the holdout split. Thus, temperature tuning is not used as a holdout model-selection signal and should be interpreted as scalar readout tuning rather than full uncertainty calibration.

Figure~\ref{fig:temperature_sensitivity} shows the corresponding development-split temperature curves. The minima differ across objectives, which is why a fixed temperature would conflate event-time objective quality with readout-scale mismatch.

\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\linewidth]{img/ch1/temperature_sensitivity.png}
    \caption{Development-split temperature sensitivity for event-time posterior readout. Vertical lines mark the selected temperatures.}
    \label{fig:temperature_sensitivity}
\end{figure}

\subsection{Posterior Geometry Diagnostics}

Once the scalar readout is fixed, the posterior can be inspected directly. We use these diagnostics to ask what temporal evidence supports each scalar prediction: whether probability mass is concentrated near the observed RT, whether the posterior is diffuse or sharply peaked, whether the posterior mode agrees with the posterior mean, and whether nominal posterior intervals have meaningful empirical coverage.

Figure~\ref{fig:posterior_raster_main} provides a trial-sorted view of the predicted posteriors before reducing them to summary metrics. Figure~\ref{fig:posterior_geometry_main} and Table~\ref{tab:posterior_geometry_metrics} summarize four complementary quantities: scalar holdout \(\tau\)-nRMSE, near-target mass within 150 ms of the observed RT, the median width of the central 80\% posterior interval, and coverage error across nominal central intervals.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{img/ch1/posterior_geometry_main.png}
    \caption{Posterior geometry differs across event-time losses even when scalar readout accuracy is similar. Panel A reports holdout scalar readout after selecting the softmax temperature on R9--R10. Panel B aligns predicted posteriors to the observed RT. Panels C--E summarize posterior width, near-target mass, and mode-mean disagreement. Panel F compares empirical coverage of central posterior intervals; the diagonal is ideal interval coverage.}
    \label{fig:posterior_geometry_main}
\end{figure}

\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{img/ch1/trial_sorted_posterior_raster.png}
    \caption{Trial-sorted event-time posterior maps on the holdout split. Rows are quantile bins of representable trials sorted by observed RT; the x-axis is time from stimulus onset; color shows log-transformed posterior density averaged within each bin. The black curve marks the mean observed RT in each bin.}
    \label{fig:posterior_raster_main}
\end{figure}

\begin{table}[!t]
\centering
\caption{Posterior-geometry summaries after readout-temperature tuning. Values are mean $\pm$ sample standard deviation across seeds. Mass $\pm$150 ms is the probability assigned within 150 ms of the observed RT; Width80 is the median 80\% central interval width; lower Coverage MAE indicates closer empirical interval coverage.}
...
\toprule
\textbf{Objective} & \textbf{Holdout \(\tau\)-nRMSE} & \textbf{Mass \(\pm150\) ms} & \textbf{Width80 ms} & \textbf{Coverage MAE} \\
\midrule
CE & 0.875 $\pm$ 0.004 & 0.334 $\pm$ 0.008 & 766 $\pm$ 36 & 0.101 $\pm$ 0.014 \\
EventNLL & 0.877 $\pm$ 0.002 & 0.490 $\pm$ 0.008 & 502 $\pm$ 15 & 0.273 $\pm$ 0.021 \\
Mixture EventNLL & 0.874 $\pm$ 0.005 & 0.471 $\pm$ 0.008 & 528 $\pm$ 22 & 0.207 $\pm$ 0.017 \\
Hazard EventNLL & 0.878 $\pm$ 0.004 & 0.502 $\pm$ 0.005 & 445 $\pm$ 14 & 0.302 $\pm$ 0.006 \\
Time-only & 0.892 $\pm$ 0.005 & 0.357 $\pm$ 0.015 & 844 $\pm$ 96 & 0.040 $\pm$ 0.024 \\
Wasserstein & 0.890 $\pm$ 0.003 & 0.334 $\pm$ 0.028 & 632 $\pm$ 101 & 0.059 $\pm$ 0.021 \\
\bottomrule
\end{tabular}
\end{table}

These summaries show that similar scalar readouts can correspond to different posterior semantics. The EventNLL-family variants assign substantially more probability mass near the observed RT and produce narrower intervals, while CE gives a competitive scalar readout with a broader posterior. Time-only and Wasserstein controls have lower coverage error, but they are weaker scalar predictors and assign less near-target mass than the CE/EventNLL group.

\subsection{Shifted-Crop Shortcut-vs-Localization Diagnostic}

Posterior geometry shows where a model places probability mass inside the standard window, but it does not prove that the model has learned a crop-relative event localizer. A fixed stimulus-locked window can still permit shortcut behavior: the model may predict trial difficulty, subject-level slowness, or stimulus-locked response tendency without moving its prediction when the crop is shifted.

We therefore evaluate trained models on multiple 2~s crops from the same 5~s holdout trials, with crop starts \(s\in\{0.2,0.3,\dots,0.8\}\) s after stimulus onset. Shifted-crop accuracy is reported as relative nRMSE on valid crop examples where the behavioral response remains observable inside the evaluated crop. Shift-tracking metrics are computed on a matched common-inside subset, so prediction changes can be attributed to the imposed crop displacement rather than to a changing trial set. Sensitivity measures the fraction of the imposed crop shift reflected in the prediction, with 1 indicating ideal crop-relative localization and 0 indicating crop-invariant behavior. Direction is the fraction of shifted examples whose prediction moves in the expected localizer direction.

\subsection{Shift-Jitter Training Intervention}

The shifted-crop diagnostic naturally suggests an intervention: if fixed-window training permits shortcut behavior, randomizing crop start during training should encourage crop-relative timing. We therefore compare fixed-window-trained and shift-jitter-trained versions of the same event-time objectives. Holdout \(\tau\)-nRMSE and shifted relative nRMSE are different metrics with different target distributions, so deltas should be interpreted within each metric family.

\begin{table}[!t]
\centering
\caption{Accuracy comparison between fixed-window-trained and shift-jitter-trained event-time models. Holdout \(\tau\)-nRMSE is the temperature-tuned fixed-window score; shifted rel nRMSE measures crop-relative accuracy over shifted crops. The two nRMSE variants use different target distributions, so deltas compare jitter training against fixed-window training within the same metric only.}
\label{tab:shifted_crop_accuracy}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\resizebox{\linewidth}{!}{%
...
\toprule
\textbf{Objective} & \multicolumn{3}{c}{\textbf{Holdout \(\tau\)-nRMSE}} & \multicolumn{3}{c}{\textbf{Shifted rel nRMSE}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
& \textbf{Fixed train} & \textbf{Jitter train} & \(\boldsymbol{\Delta}\) & \textbf{Fixed train} & \textbf{Jitter train} & \(\boldsymbol{\Delta}\) \\
\midrule
CE & 0.8753 $\pm$ 0.0039 & 0.8453 $\pm$ 0.0075 & $-0.0301$ & 0.8679 $\pm$ 0.0024 & 0.8569 $\pm$ 0.0060 & $-0.0110$ \\
Mixture EventNLL & 0.8745 $\pm$ 0.0053 & 0.8505 $\pm$ 0.0065 & $-0.0240$ & 0.8663 $\pm$ 0.0037 & 0.8576 $\pm$ 0.0041 & $-0.0087$ \\
EventNLL & 0.8772 $\pm$ 0.0018 & 0.8503 $\pm$ 0.0049 & $-0.0269$ & 0.8685 $\pm$ 0.0023 & 0.8593 $\pm$ 0.0028 & $-0.0092$ \\
Hazard EventNLL & 0.8778 $\pm$ 0.0041 & 0.8542 $\pm$ 0.0063 & $-0.0236$ & 0.8692 $\pm$ 0.0028 & 0.8609 $\pm$ 0.0045 & $-0.0083$ \\
Soft-argmax RT & 0.8917 $\pm$ 0.0046 & 0.8618 $\pm$ 0.0030 & $-0.0299$ & 0.8857 $\pm$ 0.0069 & 0.8613 $\pm$ 0.0032 & $-0.0244$ \\
Wasserstein & 0.8896 $\pm$ 0.0033 & 0.8658 $\pm$ 0.0045 & $-0.0238$ & 0.8932 $\pm$ 0.0033 & 0.8774 $\pm$ 0.0099 & $-0.0158$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Localization-behavior comparison between fixed-window-trained and shift-jitter-trained event-time models under the shifted-crop diagnostic. Deltas are jitter minus fixed-window training; positive values indicate stronger localizer-like behavior after jitter training.}
\label{tab:shifted_crop_localization}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{} l c c c c c c @{}}
\toprule
\textbf{Objective} & \multicolumn{3}{c}{\textbf{Sensitivity}} & \multicolumn{3}{c}{\textbf{Direction}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
& \textbf{Fixed train} & \textbf{Jitter train} & \(\boldsymbol{\Delta}\) & \textbf{Fixed train} & \textbf{Jitter train} & \(\boldsymbol{\Delta}\) \\
\midrule
CE & 0.5810 $\pm$ 0.0351 & 0.5831 $\pm$ 0.0406 & $+0.0021$ & 0.7780 $\pm$ 0.0041 & 0.7924 $\pm$ 0.0056 & $+0.0144$ \\
Mixture EventNLL & 0.6021 $\pm$ 0.0484 & 0.6249 $\pm$ 0.0313 & $+0.0228$ & 0.7730 $\pm$ 0.0062 & 0.7925 $\pm$ 0.0076 & $+0.0195$ \\
EventNLL & 0.5842 $\pm$ 0.0287 & 0.6174 $\pm$ 0.0222 & $+0.0332$ & 0.7739 $\pm$ 0.0059 & 0.7937 $\pm$ 0.0068 & $+0.0199$ \\
Hazard EventNLL & 0.5618 $\pm$ 0.0376 & 0.5891 $\pm$ 0.0259 & $+0.0272$ & 0.7694 $\pm$ 0.0090 & 0.7925 $\pm$ 0.0074 & $+0.0231$ \\
Soft-argmax RT & 0.5381 $\pm$ 0.0586 & 0.5775 $\pm$ 0.0362 & $+0.0395$ & 0.7592 $\pm$ 0.0183 & 0.7951 $\pm$ 0.0052 & $+0.0359$ \\
Wasserstein & 0.6684 $\pm$ 0.0711 & 0.6852 $\pm$ 0.0416 & $+0.0167$ & 0.7830 $\pm$ 0.0197 & 0.8026 $\pm$ 0.0118 & $+0.0196$ \\
\bottomrule
\end{tabular}%
}
\end{table}

Shift-jitter training consistently improves shifted-crop relative nRMSE, indicating better robustness to crop placement (Table~\ref{tab:shifted_crop_accuracy}). It also modestly improves shift-tracking behavior (Table~\ref{tab:shifted_crop_localization}), but the best sensitivity remains far below the ideal value of 1.0. Wasserstein gives the strongest sensitivity and direction, but at a clear cost in scalar and shifted-crop accuracy. These results support a partial-localization interpretation: shift-jitter reduces the fixed-window shortcut and pushes models in a more localizer-like direction, but it does not solve full crop-relative event localization.

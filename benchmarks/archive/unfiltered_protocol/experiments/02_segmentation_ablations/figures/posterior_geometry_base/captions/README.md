...
They are intentionally stored outside the figure images so manuscript wording can be edited without changing the plotted data. The current run used `base` readout and saved figure format(s): png.

Figures live in:

`benchmarks/experiments/02_segmentation_ablations/figures/posterior_geometry_base`

"""Kaggle-style meta-features for segmentation stacking.

This is a self-contained benchmark copy of the feature extractor previously
used by ``neurosned.wrappers.challenge_1``. Keep the feature semantics stable:
the stacking ablation is meant to reproduce that hand-tuned recipe from saved
benchmark artefacts.
"""
...
class MetaFeatureExtractor:
    """Build per-model temporal features from segmentation logits."""

    def __init__(
        self,
        sfreq: float = 100.0,
        win_offset: float = 0.5,
        temps: tuple[float, ...] = (0.7, 1.0, 1.3),
    ...
    def build_from_logits_store(
        self,
        seg_logits_store: dict,
        cls_outputs_store: dict | None = None,
        return_names: bool = False,
    ):
        """Build features from dicts: ``seg[name] -> (N,T)``, ``cls[name] -> (N,K or 1)``."""
        Xs = []
        names = []
        if seg_logits_store:
        ...
        if cls_outputs_store:
            for name in cls_outputs_store.keys():
                ...
                names += nms
        if not Xs:
            raise ValueError("No logits or classifier outputs were provided.")
    ...
    def build_from_batch(
        self,
        seg_logits_list: list[np.ndarray],
        cls_outputs_list: list[np.ndarray] | None = None,
        return_names: bool = False,
    ):
        """Build features from aligned model-output lists."""
        Xs = []
        names = []
